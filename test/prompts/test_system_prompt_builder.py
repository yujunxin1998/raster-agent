"""`SystemPromptBuilder` 单元测试：条件拼装七大模块。"""
...
def fake_system_dir(tmp_path: Path) -> Path:
    ...
    (agent_dir / "plan_system.md").write_text("可以建立计划。", encoding="utf-8")
...
def test_all_switches_enabled_includes_all_modules_in_order(fake_system_dir: Path) -> None:
    ...
    expected_order = [
        ...
        "<skill_system>", "<subagent_system>", "<plan_system>", "<response_style>",
    ]
...
def test_plan_disabled_omits_module_and_tag_entirely(fake_system_dir: Path) -> None:
    builder = SystemPromptBuilder(system_dir=fake_system_dir)
    result = builder.build("fake_agent", plan_enabled=False)

    assert "<plan_system>" not in result
    assert "可以建立计划" not in result
    # 其它模块不受影响
    assert "<role>" in result
...
def test_missing_module_file_is_skipped_without_raising(tmp_path: Path) -> None:
    ...
    for tag in ("thinking_style", "clarification_system", "skill_system",
                "subagent_system", "plan_system", "response_style"):
        assert f"<{tag}>" not in result


def test_lead_agent_real_modules_build_successfully() -> None:
    """回归测试：真实的 `system/lead_agent/` 七个模块文件都存在且能正常拼装。"""
