...
async def test_sub_agent_receives_isolated_config_without_callbacks() -> None:
    ...
    _, passed_config = fake_sub_agent.ainvoke.await_args.args
    assert set(passed_config.keys()) == {"configurable"}
    assert passed_config["configurable"] == {"thread_id": "c1", "secrets": {}}
